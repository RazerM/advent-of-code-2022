...
def default_day() -> int:
    today = dt.date.today()
    if today.year == 2022 and today.month == 12:
        day = today.day
    else:
        click.secho(
            "It's not december 2022, so I can't guess which day you want to run!",
            fg="yellow",
        )
        day = 25
...
def prepare() -> None:
    ...
    if Path.cwd().is_relative_to(expected_root_dir):
        ...
        click.secho(
            "Either aoc2022 is not an editable install, or you're running this "
    ...
    last_available_day = min(dt.date.today(), dt.date(2022, 12, 25)).day
